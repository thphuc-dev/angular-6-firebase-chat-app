...
export class AddAnnouncementComponent implements OnInit {
  ...
  async addItem(form: NgForm) {
    if (form.valid) {
      ...
      const announcement = await this.innowayApi.announcement.add({ createdStaffID,  announcementTypeID, announcementAttaches, title, content });
      console.log('bambi created announcement', announcement);
      this.alertAddSuccess();

      // Send FCM topic
      const urlFCM = 'https://fcm.googleapis.com/fcm/send';
      const keyFCM = environment.innoway.legacyServerKey;
      const dataFCM = {
        'to': `/topics/general_announcement`,
        'data': {
          'body': title,
          'title': title,
          'action': 'announcement',
          'announcementId': announcement && announcement.id ? announcement.id : 'cdce8e55-9614-4443-b364-4f3cda741fcc'
        }
      };
      const headerFCM = {
      ...
      const httpHeader = new HttpHeaders(headerFCM);
      const req = new HttpRequest('POST', urlFCM, dataFCM, { headers: httpHeader });
      this.http.request(req).toPromise()
        .then(res => {
          console.log('bambi - send annoucement fcm successfully', res);
        });
