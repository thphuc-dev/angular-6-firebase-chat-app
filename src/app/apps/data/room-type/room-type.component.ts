...
export class RoomTypeComponent implements OnInit {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.roomType.getList({ query }));
    for (let i = 0; i < this.items.getValue().length; i++){
      const item = this.items.getValue()[i];
      const listRoomFees = await this.innowayApi.roomType.getListRoomFeesOfRoomType(item.id);
      item['roomFees'] = listRoomFees;
    }
    this.itemCount = this.innowayApi.roomType.pagination.totalItems;
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.roomType.delete(item.id);
  ...
  async export() {
    ...
    const listData = await this.innowayApi.roomType.getList({ query });
