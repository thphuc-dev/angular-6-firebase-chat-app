...
@Component({
  selector: 'app-priority-target',
  providers: [Globals],
  templateUrl: './priority-target.component.html',
  styleUrls: ['./priority-target.component.scss']
})
export class PriorityTargetComponent implements OnInit {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.priorityTarget.getList({ query }));
    this.itemCount = this.innowayApi.priorityTarget.pagination.totalItems;
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.priorityTarget.delete(item.id);
  ...
  async export() {
    ...
    const listData = await this.innowayApi.priorityTarget.getList({ query });
