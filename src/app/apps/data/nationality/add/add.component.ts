...
export class AddComponent implements OnInit {
  ...
  async setData() {
    try {
      ...
      const data = await this.innowayApi.nationalities.getList({ query });
  ...
  async addItem(form: NgForm) {
    if (form.valid) {
      const { name} = this;
      await this.innowayApi.nationalities.add({ name});
  ...
  async updateItem(form: NgForm) {
    if (form.valid) {
      const { name } = this;
      await this.innowayApi.nationalities.update(this.id, { name });
      form.reset();
      form.resetForm(this.setDefaultData());
