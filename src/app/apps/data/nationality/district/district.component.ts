...
export class DistrictComponent implements OnInit {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.districts.getListDistricts(this.nid, this.rid, { query }));
    this.itemCount = this.innowayApi.districts.pagination.totalItems;
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.districts.deleteDistrict(this.nid, this.rid ,item.id);
      this.getItems();
  ...
  async getListRegions() {
    try {
      ...
      for (let i = 0; i < this.listRegions.length; i++){
        if (parseInt(this.listRegions[i].id, 10) === parseInt(this.rid, 10)){
          this.region = this.listRegions[i].name;
        }
      }
      this.ref.detectChanges();
    } catch (err) {
