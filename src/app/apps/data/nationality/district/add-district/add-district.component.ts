...
export class AddDistrictComponent implements OnInit {
  ...
  async setData() {
    try {
      ...
      const data = await this.innowayApi.districts.getListDistricts(this.nid, this.rid, { query });
      for (let i = 0; i < data.length; i++) {
        const item: any = data[i];
        if (parseInt(item.id, 10) === parseInt(this.did, 10)) {
          this.name = item.name;
        }
      }
    } catch (err) {
  ...
  async addItem(form: NgForm) {
    if (form.valid) {
      const { name} = this;
      await this.innowayApi.districts.addDistrict(this.nid, this.rid, { name});
  ...
  async updateItem(form: NgForm) {
    if (form.valid) {
      const { name } = this;
      await this.innowayApi.districts.updateDistrict(this.nid, this.rid, this.did ,{ name });
      form.reset();
      form.resetForm(this.setDefaultData());
