...
export class AddComponent implements OnInit {
  ...
  async setData() {
    try {
      const data = await this.innowayApi.religion.getList();
  ...
  async addItem(form: NgForm) {
    if (form.valid) {
      ...
      await this.innowayApi.religion.add({ name, valueTranslations });
  ...
  async updateItem(form: NgForm) {
    if (form.valid) {
      const { name, valueTranslations  } = this;
      await this.innowayApi.religion.update(this.id, { name, valueTranslations  });
      form.reset();
      form.resetForm(this.setDefaultData());
