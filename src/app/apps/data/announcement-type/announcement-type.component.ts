...
@Component({
  ...
  styleUrls: ['./announcement-type.component.scss']
})
export class AnnouncementTypeComponent implements OnInit {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.announcementType.getList({ query }));
    this.itemCount = this.innowayApi.announcementType.pagination.totalItems;
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.announcementType.delete(item.id);
  ...
  async export() {
    ...
    const listData = await this.innowayApi.announcementType.getList({ query });
