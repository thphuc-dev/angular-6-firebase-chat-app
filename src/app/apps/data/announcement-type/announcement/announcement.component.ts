...
export class AnnouncementComponent implements OnInit {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.announcement.getList({ query }));
    this.itemCount = this.innowayApi.announcement.pagination.totalItems;
  ...
  async deleteAll() {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++){
        const row = rows[i];
        const item =  row.item;
        await this.innowayApi.announcement.delete(item.id);
      }
  ...
  async export() {
    ...
    const listData = await this.innowayApi.announcement.getList({ query });
  ...
  async getListAnnouncementTypes() {
    try {
      this.listAnnouncementTypes = await this.innowayApi.announcementType.getList(); 
      for (let i = 0; i < this.listAnnouncementTypes.length; i++){
        if (parseInt(this.listAnnouncementTypes[i].id,10) === parseInt(this.atid,10)) {
          this.announcementType = this.listAnnouncementTypes[i].name;
        }
      }
      this.ref.detectChanges();
    } catch (err) {
