...
export class RoomComponent implements OnInit {
  ...
  async getItems() {
    ...
    this.items.next(await this.innowayApi.building.getListRooms(this.bid, { query }));
    this.itemCount = this.innowayApi.building.pagination.totalItems;
  ...
  async deleteItem(item) {
    ...
    try {
      ...
      await this.innowayApi.building.deleteRoom(this.bid , item.id);
  ...
  async export() {
    ...
    const listData = await this.innowayApi.building.getList({ query });
  ...
  async getListBuildings() {
    try {
      ...
      for (let i = 0; i < this.listBuildings.length; i++){
        if (this.listBuildings[i].id === this.bid){
          this.building = this.listBuildings[i].name;
        }
      }
      this.ref.detectChanges();
    } catch (err) {
