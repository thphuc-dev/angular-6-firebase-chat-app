...
export class AddComponent implements OnInit {
  ...
  async setData() {
    try {
      const data = await this.innowayApi.race.getList();
  ...
  async addItem(form: NgForm) {
    if (form.valid) {
      ...
      await this.innowayApi.race.add({ name, valueTranslations });
  ...
  async updateItem(form: NgForm) {
    if (form.valid) {
      const { name, valueTranslations  } = this;
      await this.innowayApi.race.update(this.id, { name, valueTranslations  });
      form.reset();
      form.resetForm(this.setDefaultData());
